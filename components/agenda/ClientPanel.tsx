...
type Section = "dashboard" | "appointments" | "services" | "staff" | "settings";
...
export function ClientPanel() {
  ...
  const nav = [
    ...
    { label: "Link publico", active: section === "settings", onClick: () => setSection("settings") },
  ];
  ...
  return (
    ...
    >
      <header className="mb-6 flex items-start justify-between gap-4 max-md:flex-col">
        <div>
          <p className="font-bold text-muted">Painel do cliente</p>
  ...
  return (
    <>
      <div className="grid grid-cols-4 gap-4 max-xl:grid-cols-2 max-md:grid-cols-1">
        ...
        <MetricCard label="Servicos ativos" value={store.metrics.activeServices} tone="rust" />
      </div>
...
function Settings({ store }: { store: ReturnType<typeof useAgendaStore> }) {
  ...
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_380px] gap-4 max-lg:grid-cols-1">
      <Panel>
        <h3 className="mb-4 text-lg font-black">Dados do negocio</h3>
        <form onSubmit={submit} className="grid gap-3">
          <label>
            Nome
